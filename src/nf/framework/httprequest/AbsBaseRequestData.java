...
import nf.framework.demo.DemoVO;
...
public abstract class AbsBaseRequestData<T> {
	...
	public AbsBaseRequestData(Context mcontext,boolean isCacheData) {
		if(mcontext==null){
			throw  new RuntimeException("mcontext can not be empty!");
		}
	...
	protected void run() {
		try {
			if(mHttpRequestInterface==null){
				throw new RuntimeException("HTTP request must implements HttpRequestInterface");	
			}
	...
	private void httpPostByteData(NetworkRequest mNetworkRequest2) {
		...
		if(mHttpRequestInterface==null){
			throw new RuntimeException("HTTP post byte request must implements HttpPostByteRequestInterface");
		}
	...
	private void httpPost(NetworkRequest mNetworkRequest) {
		// TODO Auto-generated method stub
		if(mHttpRequestInterface==null){
			throw new RuntimeException("HTTP post request must implements HttpPostRequestInterface");
		}
		String url =mHttpRequestInterface.bulidUrl();
		String responseData = null;
		...
		if(mHttpRequestInterface instanceof HttpPostFilesRequestInterface){
			...
			if((mPostParams==null||mPostParams.isEmpty())&&(mPostFiles==null||mPostFiles.isEmpty())){
				throw new RuntimeException("HTTP post file request must contain a param map or a file map !");
			}
			...
			if(mPostParams==null||mPostParams.isEmpty()){
				throw new RuntimeException("HTTP post request must contain a param map !");
			}
	...
	private void httpGet(NetworkRequest mNetworkRequest) {
		// TODO Auto-generated method stub
		if(mHttpRequestInterface==null){
			throw new RuntimeException("http get Request method must implements HttpGetRequestInterface");
		}
		String url =mHttpRequestInterface.bulidUrl();
		String responseData = null;
	...
	protected  void saveCache(String jsonData){
		...
		if(isCacheData&&!TextUtils.isEmpty(jsonData)){
			...
			if(TextUtils.isEmpty(cacheName)){
				throw new RuntimeException(" cacheFileName is null");
			}
